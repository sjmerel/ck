...
void MD5::MD5Init(MD5_CTX* mdContext)
{
...
MD5::MD5()
{
    Mem::clear(m_hash, sizeof(m_hash));
    MD5Init(&m_context);
}

void MD5::update(const byte* data, int length)
{
  MD5Update(&m_context, (unsigned char*) data, length);
}

void MD5::finish()
{
  MD5Final(&m_context);
  Mem::copy(m_hash, m_context.digest, sizeof(m_hash));
}
